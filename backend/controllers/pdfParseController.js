...
function parseUnicreditStatement(text) {
  ...
  const skipPatterns = [
    /BAM/i, /stanje/i, /broj racuna/i, /datum/i, /opis prometa/i,
    /iznos prometa/i, /novo stanje/i, /prethodno stanje/i,
    /promet po racunu/i, /izvod po/i, /glavinic/i, /unicredit/i,
    /reklamacije/i, /transakcijski/i, /dopušteno/i, /kamatna/i,
    /mjesecna/i, /postovani/i, /zahvaljujemo/i, /informiramo/i,
    /aktivnosti/i, /^www\./i, /^info@/i, /^tel:/i, /^080/i,
    /^\d{1,2}$/, /^-?\d+\.\d{2}$/, /^\d{2}\.\d{2}\.\d{4}$/
  ];
  ...
  const transactions = [];
  const maxTransactions = Math.min(dates.length, transactionAmounts.length, merchantNames.length);
...
exports.parseUnincreditBankPdf = async (req, res) => {
    try {
        ...
        if (transactions.length === 0) {
            return res.status(200).json({ transactions: [], message: "No transactions found. Check PDF format." });
        }
